...
def get_hist(path, tree_name, xmin, xmax, xbins, color):
    ...
    h_data = FSModeHistogram.getTH1F(path, tree_name, '', 'MASS([K+], [K-])', '(' + str(xbins) + ', ' + str(xmin) + ', ' + str(xmax) + ')', 'CUT(rank, EnPB, Vz, Chi2DOF, NUMTRK, range_Lambdac, range_D, Lambda_1520, Delta_1235)')
    xtitle = 'M(K^{+}K^{-}) (MeV/#it{c}^{2})'
...
def draw(run_periods, versions, xmin, xmax, xbins):
    ...
    Arr_low = set_arrow(1.05, 0, 1.05, max * 0.8, 0.02, 2, '<', 2)
    Arr_low.Draw()

    if not os.path.exists('./figs/'): os.makedirs('./figs/')
    mbc.SaveAs('./figs/m_KpKm.pdf')
