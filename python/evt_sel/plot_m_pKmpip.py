...
sys.path.append('../')
from tools import set_canvas_style, set_pub_style, set_prelim_style, format_data_hist, name_axis, set_legend, format_mc_hist, set_yzero_hist
set_pub_style()
set_prelim_style()
TGaxis.SetMaxDigits(3)

"""
event selection: draw M(pK+pi-)
"""

__author__    = "JING Maoqiang <jingmq@ihep.ac.cn>"
__copyright__ = "Copyright (c) JING Maoqiang"
__created__   = "[2024-07-17 Wed 16:00]"
...
def get_hist(path, tree_name, xmin, xmax, xbins, color):
    ...
    cut_range_D       = 'MASS([K+], [pi-]) > 1.8 && MASS([K+], [pi-]) < 1.9'
    ...
    h_data = FSModeHistogram.getTH1F(path, tree_name, '', 'MASS([p+], [K-], [pi+])', '(' + str(xbins) + ', ' + str(xmin) + ', ' + str(xmax) + ')', 'CUT(rank, EnPB, Vz, Chi2DOF, NUMTRK, range_Lambdac, range_D, phi_1020, Lambda_1520, Delta_1235)')
...
def draw(run_periods, versions, xmin, xmax, xbins):
    ...
    for data_key, side_key, mc_key, p2K2pi_key, p4pi_key, ws_key in zip(data_hist_mg.keys(), side_hist_mg.keys(), mc_hist_mg.keys(), p2K2pi_hist_mg.keys(), p4pi_hist_mg.keys(), ws_hist_mg.keys()):
        set_yzero_hist(data_hist_mg[data_key])
