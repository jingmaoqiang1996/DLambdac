...
sys.path.append('../')
from tools import set_canvas_style, set_pub_style, set_prelim_style, format_data_hist, name_axis, set_legend, format_mc_hist, set_arrow
set_pub_style()
set_prelim_style()
TGaxis.SetMaxDigits(3)

"""
event selection: draw M(pK-)
"""

__author__    = "JING Maoqiang <jingmq@ihep.ac.cn>"
__copyright__ = "Copyright (c) JING Maoqiang"
__created__   = "[2024-07-17 Wed 16:25]"
...
def get_hist(path, tree_name, xmin, xmax, xbins, color):
    ...
    h_data = FSModeHistogram.getTH1F(path, tree_name, '', 'MASS([p+], [K-])', '(' + str(xbins) + ', ' + str(xmin) + ', ' + str(xmax) + ')', 'CUT(rank, EnPB, Vz, Chi2DOF, NUMTRK, range_Lambdac, range_D, phi_1020, Delta_1235)')
    xtitle = 'M(pK^{-}) (MeV/#it{c}^{2})'
...
def draw(run_periods, versions, xmin, xmax, xbins):
    ...
    Arr_low = set_arrow(1.5, 0, 1.5, max * 0.8, 0.02, 2, '<', 2)
    Arr_low.Draw()

    Arr_up = set_arrow(1.54, 0, 1.54, max * 0.8, 0.02, 2, '<', 2)
    Arr_up.Draw()

    if not os.path.exists('./figs/'): os.makedirs('./figs/')
    mbc.SaveAs('./figs/m_pKm.pdf')
