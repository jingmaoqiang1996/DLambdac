...
def fit(data, sig_path):
    ...
    while True:
        ...
        bkg_list = ['bkgpdf_m_Kppim_bkgpdf_m_pKmpip']
        colors = [2, 3]
        lines  = [1, 10]
        n_comp = [
        ...
        legend_comp = set_legend(h_comp, n_comp, s_comp, 0.68, 0.7, 0.78, 0.8, 2, size = 0.05)
        legend_comp.Draw()
        ...
        legend_comp = set_legend(h_comp, n_comp, s_comp, 0.68, 0.3, 0.78, 0.4, 2, size = 0.05)
        legend_comp.Draw()
    ...
    with open('./txts/likelihood_m_Kppim_m_pKmpip_fit_no_sig.txt', 'w') as f:
