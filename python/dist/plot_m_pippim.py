...
def get_hist(path, tree_name, xmin, xmax, xbins, color):
    ...
    h_data = FSModeHistogram.getTH1F(path, tree_name, '', 'MASS([pi+], [pi-])', '(' + str(xbins) + ', ' + str(xmin) + ', ' + str(xmax) + ')', 'CUT(rank, EnPB, Vz, Chi2DOF, NUMTRK, range_Lambdac, range_D, phi_1020, Lambda_1520, Delta_1235)')
    xtitle = 'M(#pi^{+}#pi^{-}) (MeV/#it{c}^{2})'
