...
__created__   = "[2024-07-17 Wed 17:09]"
...
def get_hist(path, tree_name, xmin, xmax, xbins, color):
    ...
    h_data = FSModeHistogram.getTH1F(path, tree_name, '', 'MASS([K+], [pi+], [pi-])', '(' + str(xbins) + ', ' + str(xmin) + ', ' + str(xmax) + ')', 'CUT(rank, Vz, EnPB, Chi2DOF, NUMTRK, range_Lambdac, range_D, phi_1020, Lambda_1520, Delta_1235)')
...
def draw(run_periods, versions, xmin, xmax, xbins):
    ...
    mbc.SaveAs('./figs/m_Kppippim.pdf')
