...
def get_hist(path, tree_name, xmin, xmax, xbins, color):
    ...
    h_data = FSModeHistogram.getTH1F(path, tree_name, '', 'MASS([p+], [pi+], [pi-])', '(' + str(xbins) + ', ' + str(xmin) + ', ' + str(xmax) + ')', 'CUT(rank, Vz, EnPB, Chi2DOF, NUMTRK, range_Lambdac, range_D, phi_1020, Lambda_1520, Delta_1235)')
    xtitle = 'M(p#pi^{+}#pi^{-}) (MeV/#it{c}^{2})'
...
def draw(run_periods, versions, xmin, xmax, xbins):
    ...
    # Arr = set_arrow(1.8, 0, 1.8, max * 0.8, 0.02, 2, '<', 2)
    # Arr.Draw()

    if not os.path.exists('./figs/'): os.makedirs('./figs/')
    mbc.SaveAs('./figs/m_ppippim.pdf')
