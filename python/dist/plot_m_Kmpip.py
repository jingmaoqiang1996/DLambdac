...
__created__   = "[2024-07-17 Wed 16:08]"
...
def get_hist(path, tree_name, xmin, xmax, xbins, color):
    ...
    h_data = FSModeHistogram.getTH1F(path, tree_name, '', 'MASS([K-], [pi+])', '(' + str(xbins) + ', ' + str(xmin) + ', ' + str(xmax) + ')', 'CUT(rank, EnPB, Vz, Chi2DOF, NUMTRK, range_Lambdac, range_D, phi_1020, Lambda_1520, Delta_1235)')
    xtitle = 'M(K^{-}#pi^{+}) (MeV/#it{c}^{2})'
...
def draw(run_periods, versions, xmin, xmax, xbins):
    ...
    # Arr_low = set_arrow(0.8, 0, 0.8, max * 0.8, 0.02, 2, '<', 2)
    # Arr_low.Draw()

    # Arr_up = set_arrow(1.0, 0, 1.0, max * 0.8, 0.02, 2, '<', 2)
    # Arr_up.Draw()

    if not os.path.exists('./figs/'): os.makedirs('./figs/')
    mbc.SaveAs('./figs/m_Kmpip.pdf')
